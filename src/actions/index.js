export function getFavoriteTrails() {
  return (dispatch) => {
    ...
    var proxyUrl = '',
      targetUrl = 'https://katy-hiking-trails.fly.dev/trails/favorites.json';
...
export function persistTrail(name, description, link, comments) {
  ...
  return (dispatch) => {
    ...
    var proxyUrl = '',
      targetUrl = 'https://katy-hiking-trails.fly.dev/trails/favorites';
    return fetch(proxyUrl + targetUrl, {
      method: 'post',
      body: noteInfo,
      headers: {
      ...
      .then((payload) => dispatch({ type: 'ADD_TRAIL', payload }));
  };
}
...
export function updateTRAIL(trailId, name, description, link, comments) {
  return (dispatch) => {
    dispatch({ type: 'SAVING_TRAIL' });
    var proxyUrl = '',
      targetUrl = `https://katy-hiking-trails.fly.dev/trails/favorites/${trailId}`;
    return fetch(proxyUrl + targetUrl, {
      method: 'put',
      body: JSON.stringify({
        note: {
      ...
      .then((payload) => {
        dispatch({ type: 'LOADING_FAVORITE_TRAILS' });
        return fetch(
          proxyUrl + 'https://katy-hiking-trails.fly.dev/trails/favorites',
          {
            method: 'get',
            headers: {
              'Content-Type': 'application/json',
          ...
          .then((payload) =>
            dispatch({ type: 'SHOW_FAVORITE_TRAILS', payload })
          );
      });
  };
}
...
export function getTrails() {
  return (dispatch) => {
    ...
    var proxyUrl = '',
      targetUrl = 'https://katy-hiking-trails.fly.dev/trails.json';
    return fetch(proxyUrl + targetUrl, {
      method: 'get',
...
export function getAddicksTrails() {
  return (dispatch) => {
    dispatch({ type: 'LOADING_TRAILS' });
    var proxyUrl = '',
      targetUrl = 'https://katy-hiking-trails.fly.dev/trails/addicks.json';
    return fetch(proxyUrl + targetUrl, {
      method: 'get',
      headers: { 'Content-Type': 'application/json' },
    })
...
export function getBarkerTrails() {
  return (dispatch) => {
    dispatch({ type: 'LOADING_TRAILS' });
    var proxyUrl = '',
      targetUrl = 'https://katy-hiking-trails.fly.dev/trails/barker.json';
    return fetch(proxyUrl + targetUrl, {
      method: 'get',
      headers: { 'Content-Type': 'application/json' },
    })
...
export function jwt(data, routerHistory) {
  return (dispatch) => {
    ...
    var proxyUrl = '',
      targetUrl = 'https://katy-hiking-trails.fly.dev/user_token';
    return fetch(proxyUrl + targetUrl, {
      method: 'POST',
      headers: {
      ...
      .then((response) => {
        if (response.status >= 200 && response.status < 300) {
          response.json().then((data) => {
            ...
            dispatch({ type: 'LOADING' });
            var targetUrl1 = 'https://katy-hiking-trails.fly.dev/api/users/:id';
...
export function signUp(data, routerHistory) {
  return (dispatch) => {
    dispatch({ type: 'LOADING' });
    var proxyUrl = '',
      targetUrl = 'https://katy-hiking-trails.fly.dev/api/register';
    return fetch(proxyUrl + targetUrl, {
      method: 'POST',
      headers: {
        Accept: 'application/json',
